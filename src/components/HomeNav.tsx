...
const SECTIONS = [
  { id: 'hero-section', label: 'Gateway', icon: Compass, desc: 'Intro & Origins' },
  { id: 'challenge-section', label: 'Challenge', icon: Lock, desc: 'Traditional Struggles' },
  { id: 'story-section', label: 'Chronicle', desc: 'Our Organic Journey', icon: BookOpen },
  { id: 'insights-section', label: 'The Pulse', desc: 'Discord Live Status', icon: Users },
  { id: 'team-teaser-section', label: 'Organizers', desc: 'Minds Behind 3een', icon: Sparkles },
  { id: 'contributor-teaser-section', label: 'Contribute', desc: 'Become a Contributor', icon: Heart },
];
...
export default function HomeNav() {
  ...
  useEffect(() => {
    const observerOptions = {
      root: null,
      rootMargin: '-30% 0px -50% 0px', // Helps isolate section center
      threshold: 0,
    };
        ...
        <AnimatePresence>
          {showScrollTop && (
            <motion.button
              ...
              onClick={scrollToTop}
              className="w-8 h-8 rounded-full bg-primary/10 border border-primary/20 backdrop-blur-md text-primary hover:text-white hover:bg-primary flex items-center justify-center transition-all cursor-pointer shadow-md border-emerald-500/30"
              title="Return to top"
            >
              <ChevronUp size={13} />
            </motion.button>
            ...
            <>
              ...
              >
                <div className="flex items-center justify-between border-b border-border/40 dark:border-white/5 pb-3.5 mb-4">
                  <div>
                    <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-primary dark:text-emerald-400">Project 3een Gateway</span>
